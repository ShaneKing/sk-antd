...
export default class Comp extends React.Component {
  ...
  static propTypes = {
    ...
    skSysModel: React.PropTypes.instanceOf(Model),//System Model(whole of page), PlainObject or Brad.Model
  ...
  _updateUI = this.updateUI.bind(this);
  monitors = [];
  ...
  updateUI(evt) {
    this.setState({stateUid: _.uniqueId('compStateUid')});
  }
  ...
  skProp(prop, defaultValue = undefined) {
    ...
    if (rtn === undefined) {
      rtn = this.props[Comp.SK_PROPS_PREFIX + SK.upperWordFirstChar(prop)]
    }
    if (rtn === undefined) {
      rtn = this.props[Comp.SK_PROPS_PREFIX + 'Sys' + SK.upperWordFirstChar(prop)]
    }
    if (rtn === undefined) {
      rtn = Cfg['DEFAULT' + SK.upperWordFirstChar(prop).replace(/[A-Z]/g, ($1) => {
